  ...
  return (
    <section className="relative py-12 sm:py-16 bg-blue-50">
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6">
        ...
        {/* Mobile: Single testimonial, Desktop: 3-up carousel */}
        <div className="mt-6 sm:mt-8">
          {/* Mobile: Single testimonial */}
          <div className="block sm:hidden">
            <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 min-h-[280px] flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-1 text-primary-600">
                  {Array.from({ length: 5 }).map((_, star) => (
                    <svg key={star} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className={`h-5 w-5 ${star < (items[index].rating || 5) ? '' : 'text-gray-300'}`}>
                ...
                <p className="mt-4 text-base text-gray-700 leading-relaxed">"{items[index].text}"</p>
              </div>
              <div className="pt-5 text-sm text-gray-500">— {items[index].name}</div>
            </div>
          </div>
